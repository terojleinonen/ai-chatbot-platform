...
/**
 * Limits failed logins with a sliding window, per (client IP + username) and per client IP.
 * <p>
...
public class LoginRateLimiter {
    ...
    @Autowired
    public LoginRateLimiter(@Value("${security.login-rate-limit.max-failures-per-user-and-ip}") int maxPerUserAndIp,
                            @Value("${security.login-rate-limit.max-failures-per-ip}") int maxPerIp,
                            @Value("${security.login-rate-limit.window}") Duration window) {
        this(maxPerUserAndIp, maxPerIp, window, Clock.systemUTC());
    }

    LoginRateLimiter(int maxPerUserAndIp, int maxPerIp, Duration window, Clock clock) {
    ...
    public synchronized Optional<Duration> tryAcquire(String ip, String username) {
        ...
        String userKey = userKey(ip, username);
    ...
    public synchronized void recordSuccess(String ip, String username) {
        ...
        Deque<Instant> ipAttempts = attempts.get(ipKey(ip));
        if (ipAttempts != null) {
            ipAttempts.pollLast();
            if (ipAttempts.isEmpty()) attempts.remove(ipKey(ip));
        }
    }
